...
module.exports = class AuthService extends Service {
  sign(app, payload = {}, isLogin = true) {
    return jwt.sign(payload, app.config.secrets, {
      expiresIn: isLogin ? app.config.session.maxAge : 0
    })
  }
  ...
  async updateSessionUser(admin) {
    this.ctx.session._user =
      admin ||
      (await this.service.user.getItemById(
        this.ctx.session._user._id,
        '-password'
      ))
